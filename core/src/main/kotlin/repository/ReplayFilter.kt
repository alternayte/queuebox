 ...
 * because a replay of every row is never an accident. The outbox row carries no source or
 * destination column, so the field that narrows a replay to one kind of message is `topic`.
 */
@Serializable
data class ReplayFilter(
    ...
    fun isEmpty(): Boolean =
        createdAfter == null && createdBefore == null && topic == null && ids.isNullOrEmpty()
}
