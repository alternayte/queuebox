...
-- DECISION: do not change the migration strategy now. The online alternative for PostgreSQL
-- is CREATE INDEX CONCURRENTLY, run outside a transaction block. See
-- docs/development/migrations.md.

-- The SET QUOTED_IDENTIFIER requirement for a filtered index applies to SQL Server, not to
-- PostgreSQL: PostgreSQL has no equivalent setting, and this note is recorded here only for
-- symmetry with the SQL Server migration of the same name.
