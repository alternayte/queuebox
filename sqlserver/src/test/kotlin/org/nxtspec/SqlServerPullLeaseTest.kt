...
@Tag("integration")
class SqlServerPullLeaseTest : SqlServerTestBase() {
    private val repository = SqlServerInboxRepository()
    private val outbox = SqlServerOutboxRepository()
    ...
    private fun statement(connection: Connection, action: String, values: Map<String, Any?>): PreparedStatement {
        ...
        return connection.prepareStatement(sql).also { stmt ->
            names.forEachIndexed { index, name ->
                ...
                stmt.setObject(index + 1, if (value is UUID) value.toString() else value)
            }
        }
    }
