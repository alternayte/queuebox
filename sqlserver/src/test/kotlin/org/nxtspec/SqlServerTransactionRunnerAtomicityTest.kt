 ...
 * runner then loses the all or nothing property.
 */
@Tag("integration")
class SqlServerTransactionRunnerAtomicityTest : SqlServerTestBase() {

    private lateinit var runner: SqlServerTransactionRunner
    private lateinit var outboxRepository: SqlServerOutboxRepository
    private lateinit var inboxRepository: SqlServerInboxRepository

    @BeforeEach
    fun setup() {
        runner = SqlServerTransactionRunner()
    ...
    private fun countOutboxRows(): Long = transaction {
        SqlServerOutboxTable.selectAll().count()
    }
