...
| Enumeration | `transform.onError`, `retention.*.policy`, `auth.signaturePayloadFormat` | Write the value in the exact letter case that the tables below show. `onError: fail` does not load. |
...
| `hmac` | `signaturePayloadFormat` | enumeration | see below | `BODY` signs the body. `TIMESTAMP_DOT_BODY` signs the timestamp, a dot and the body. |
