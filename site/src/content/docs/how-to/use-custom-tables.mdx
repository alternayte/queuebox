...
description: Point QueueBox at tables and columns with your own names, or run the bundled migrations with a privileged user when QueueBox has no rights to change the schema.
sidebar:
  order: 8
---
...
This guide shows how to run QueueBox against tables that you create and name yourself. It also shows how to apply the default schema when the QueueBox database user has no rights to change the schema.
...
Set `database.migrate: false` in two cases:

1. The QueueBox database user has no rights to create or change tables. Run the `migrate` command with a privileged user. See [Apply the default schema without DDL rights](#apply-the-default-schema-without-ddl-rights).
2. The configuration renames a table or a column. The bundled files use the default names, so they cannot create your schema. Apply your own schema. See [Rename tables and columns](#rename-tables-and-columns).
...
## Apply the default schema without DDL rights

Run the `migrate` command of the image that you deploy, with a privileged user. It reads the same configuration as the service, applies every migration that the database does not have, logs the versions, and exits.

```bash
docker run --rm \
  -e QUEUEBOX_DATABASE_URL=jdbc:postgresql://db.internal:5432/queuebox \
  -e QUEUEBOX_DATABASE_USERNAME=queuebox_admin \
  -e QUEUEBOX_DATABASE_PASSWORD=${ADMIN_PASSWORD} \
  ghcr.io/alternayte/queuebox:0.5.0 migrate
```

The command exits with 0 after success and with a non-zero code after a failure. It ignores `database.migrate`. It refuses a configuration that renames a table or a column, because the bundled files name the default schema.

These are the migrations of the current release:
...
Then turn the startup migration off, and run the service with the application user:
...
A new QueueBox version can add a migration. Run the `migrate` command of the new image before you start that version. [Upgrade QueueBox](/how-to/upgrade/) lists the steps.
