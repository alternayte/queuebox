...
- [x] **Step 1: Write the failing test.** `ComposeEnvExampleTest` parses `docker-compose.yml` and
...
- [x] **Step 2: Run it.** `./gradlew :app:test --tests '*ComposeEnvExampleTest*'`. Expect a failure
      that names `DB_URL` and `ANTHROPIC_API_KEY`.
- [x] **Step 3: Rewrite `.env.example`.** Include only QueueBox variables, plus one commented
      example each for a destination, a route and a source, taken from the shipped
      `config/src/main/resources/queuebox.yml`.
- [x] **Step 4: Correct both Compose files.** Use `QUEUEBOX_DATABASE_URL`,
...
- [x] **Step 5: Run the test.** Expect a pass.
- [x] **Step 6: Prove the DoD by hand.** Run
...
- [x] **Step 7: Add the CI job.** A `compose` job in `.github/workflows/ci.yml` that runs the same
      sequence and fails on a non-success response.
- [x] **Step 8:** `./gradlew check`. Then commit.
...
- [x] **Step 1: Write the failing test.** `ExternalConfigFileTest` writes a temporary YAML file
...
- [x] **Step 2: Run it.** `./gradlew :config:test --tests '*ExternalConfigFileTest*'`. Expect the
      first test to fail, because the packaged value is returned.
- [x] **Step 3: Implement.** Add an `addFileSource(file, optional = true)` for the resolved
...
- [x] **Step 4: Run the test.** Expect all three to pass.
- [x] **Step 5:** `./gradlew check`. Then commit.
...
- [x] **Step 1: Write the failing test.** Extend `InboxRoutesTest` with a test that posts a new
...
- [x] **Step 2: Run it.** `./gradlew :inbox-service:test --tests '*InboxRoutesTest*'`. Expect a
      failure reporting 200 where 202 is expected.
- [x] **Step 3: Implement.** Change line 121 to `HttpStatusCode.Accepted`. Leave the duplicate
      branch at 200.
- [x] **Step 4: Run the test.** Expect a pass. Then run `./gradlew :app:test` too, because the end
      to end tests assert the inbox response.
- [x] **Step 5: Record the change.** Add a `### Breaking changes` heading under the first release
...
- [x] **Step 6:** `./gradlew check`. Then commit.
...
- [x] **Step 1: Write the failing state test.** `DocumentedStateSetTest` reads
...
- [x] **Step 2: Write the failing provider test.** `DatabaseProviderFactoryTest` calls
...
- [x] **Step 3: Run both.** `./gradlew :core:test --tests '*DocumentedStateSetTest*' --tests
...
- [x] **Step 4: Decide the state machine.** `canTransitionTo` has no caller. Either call it from
...
- [x] **Step 5: Implement the provider error.** Catch `ClassNotFoundException` at both sites and
      throw `MissingDatabaseProviderException`.
- [x] **Step 6: Write `docs/architecture.md`.** It must hold a Mermaid module graph, a Mermaid
...
- [x] **Step 7: Run both tests.** Expect a pass. Then `./gradlew check`. Then commit.
...
- [x] **Step 1: Write the failing test.** `IntegrationDocSqlTest` parses `docs/integration.md`,
...
- [x] **Step 2: Run it.** `./gradlew :app:test --tests '*IntegrationDocSqlTest*'`. Expect a failure
      on the missing file.
- [x] **Step 3: Write `docs/integration.md`.** The finding names five required parts. Write all
      five.
...
- [x] **Step 4: Run the test.** Expect a pass.
- [x] **Step 5:** `./gradlew check`. Then commit.
...
- [x] **Step 1: Write the failing test.** `ReadmeStructureTest` asserts four things. `README.md` is
...
- [x] **Step 2: Run it.** `./gradlew :app:test --tests '*ReadmeStructureTest*'`. Expect a failure
      reporting 1142 lines.
- [x] **Step 3: Move the reference sections.** `## Configuration` to `docs/configuration.md`.
...
- [x] **Step 4: Write the new README.** What QueueBox is, using the verbatim positioning statement
      from `hardening-doc.md` line 96. A 60 second quick start. A `## Guarantees` section. A link
      list to every document. Nothing else.
- [x] **Step 5: Write `## Guarantees`.** F-085 names five items. Write all five as short
...
- [x] **Step 6: Correct the roadmap.** Attach a target version to each item, or mark it
      `Not planned`. Remove every item that Phase 3 and Phase 4 implemented.
- [x] **Step 7: Correct the schema section.** Document the state set that Task 4 proved, and
      `VARCHAR(50)`, not `VARCHAR(20)`. Document every inbox response code from Task 3.
- [x] **Step 8: Run the test.** Expect a pass. Then `./gradlew check`. Then commit.
...
- [x] **Step 1: Write the failing test.** `MetricsDocTest` starts the application against a
...
- [x] **Step 2: Run it.** Expect a failure on the missing file.
- [x] **Step 3: Write `docs/operations/metrics.md`.** Document the 23 `queuebox_*` metrics, the
...
- [x] **Step 4: Run the test.** Expect a pass. Then `./gradlew check`. Then commit.
...
- [x] **Step 1: Write the smoke test of the first example.** `smoke-test.sh` must start the
...
- [x] **Step 2: Run it.** Expect a failure, because no `queuebox.yml` exists yet.
- [x] **Step 3: Write the first example.** A webhook receiver: an inbox source that accepts a
      webhook and forwards it to an HTTP destination.
- [x] **Step 4: Run the smoke test.** Expect a pass.
- [x] **Step 5: Repeat step 1 to step 4 for `http-fanout`.** One outbox topic that matches two HTTP
      destinations.
- [x] **Step 6: Repeat step 1 to step 4 for `rabbitmq-bridge`.** One outbox topic that publishes to
      a RabbitMQ exchange.
- [x] **Step 7: Correct the manual setup.** F-075: `docs/getting-started.md` must use the
...
- [x] **Step 8: Add the CI job.** An `examples` job that runs all three smoke tests and the manual
      setup script.
- [x] **Step 9:** `./gradlew check`. Then commit.
...
- [x] **Step 1:** Add a build status badge, a coverage badge, a license badge and a latest release
      badge to `README.md`.
- [x] **Step 2:** Add a `## Support` section, or `SUPPORT.md` linked from the README, stating the
      support policy and the maintenance status.
- [x] **Step 3:** Check every badge URL resolves. A badge for a workflow that has never run cannot
...
- [x] **Step 4:** Add the repository description and the topics. That needs the GitHub settings,
