...
object ConfigValidator {
    ...
    fun validate(config: QueueBoxConfig): QueueBoxConfig {
        ...
        config.destinations.forEach { (name, dest) ->
            ...
            if (dest is DestinationConfig.RabbitMQ) {
                require(dest.deliveryMode in setOf("persistent", "transient")) {
                    "Destination '$name' deliveryMode must be persistent or transient, not " +
                        "'${dest.deliveryMode}'. " + setVia("destinations.$name.deliveryMode")
                }
            }
        }
        ...
        ColumnMappingValidator.validate(config.database.columnMapping)
     ...
     */
    internal val SQL_IDENTIFIER_REGEX = Regex("^[a-zA-Z_][a-zA-Z0-9_]*$")
